...

{
	public void surface(output color Ci, Oi)
	{
		...
		_diff = diffOrenNayar(Nn,V,diffRoughness)*_diffTex;
		...
		//_refRoughTex = color texture(reflRoughTex);
		//Ci = _refRoughTex;		
		if (DoPho==1) Ci = (1-occ)*(_diff + ambient()) + _spec*_specInt + _refl*_refIntTex + _refr;
		else Ci = (1-occ)*(_diff + ambient() + _indirect) + _spec*_specInt + _refl*_refIntTex + _refr;
		//Oi = 1;
		//Ci*=Oi;
	}
