...
surface fu_3de_phong( 
	float	Kd		= .5; //diffuse amount
	float   Ks		= .5; //specular amount
	...
	
{
	...
	float occ = occlusion(P,Nf,occ_samples,"maxdist", maxdist);
	...
	Oi = Os ;
	//Ci = Cs*Oi;	
	Ci = Oi * ( Ci * (Kd*diffuse(Nf)) + spec * specular(Nf, normalize( -I ), (roughness)));
	//Ci = (1-occ)* Oi * ( Ci * (Ka*ambient() + Kd*diffuse(Nf)) + spec * specular(Nf, normalize( -I ), (roughness)));
}
