...
#define MODULENAME "3DImagePlane"

#define BaseClass Transform3DOperator
#define ThisClass ImagePlane3D
...
FuRegisterClass(COMPANY_ID_DOT + CLSID_ImagePlane3D, CT_3DFilterSource)		// for internal eyeon builds COMPANY_ID_DOT/COMPANY_ID_SPACE are ""
	REGS_Name,					COMPANY_ID_SPACE "RIB Archive Loader",
	REGS_Category,				COMPANY_ID_DBS "3D",
	REG_TileID,					IDB_TILE_3D_IMAGE,
	REGS_OpIconString,		"RIB_LD",
	REGS_OpDescription,		"Reads a RIB archive and send it the RIB Renderer",
...
ImagePlane3D::ImagePlane3D(const Registry *reg, const ScriptVal &table, const TagList &tags) : BaseClass(reg, table, tags)
{
	// add the plane's geometry inputs to the controls page
	AddControlPage("Controls");

	if (InSurfacePlane = (SurfacePlaneInputs3D *) ClassRegistry->New(COMPANY_ID_DOT + CLSID_Surface_Plane_Inputs, CT_SurfaceInputs3D, table, tags))
	{
...
void ImagePlane3D::CheckRequest(Request *req)
{
...
Node3D *ImagePlane3D::CreateScene(Request *req, TagList *tags)
{
	...
	if (planeNode)
	{
		...
		SurfacePlaneData3D *surface = (SurfacePlaneData3D *) InSurfacePlane->Process(req, NULL);
...
bool ImagePlane3D::OnConnect(Link *src, Link *dest, TagList *tags)
{
